...
import type { PersonRecord, PersonAnswer } from "../shared/types.js";
import { JOB_TAGS } from "../shared/types.js";
import { HUB_API_KEY, hubVerify } from "../shared/hub.js";

const ANTHROPIC_API_KEY = process.env.ANTHROPIC_API_KEY ?? "";

const peopleConfig = {
  data_file: "hub/tasks/shared/data/people.csv",
  hub_data_url_template: `${process.env.HUB_BASE_URL ?? ""}/data/{apiKey}/people.csv`,
  current_year: 2026,
  min_age: 20,
  max_age: 40,
...
async function tagJobsWithLLM(client: Anthropic, people: PersonRecord[]): Promise<Map<number, string[]>> {
  if (people.length === 0) return new Map();

  const listText = people.map((p, i) => `${i}. ${p.job || "(brak opisu)"}`).join("\n");
...
Zwróć obiekt z polem "items" - tablicą obiektów z polami "index" (numer z listy, 0-based) i "tags" (tablica stringów - wyłącznie tagi z listy).`,
    messages: [{ role: "user", content: `Przypisz tagi do poniższych stanowisk:\n\n${listText}` }],
    output_config: { format: zodOutputFormat(BatchTaggingSchema) },
  });

  const parsed = response.parsed_output;
  if (!parsed?.items) throw new Error("LLM did not return valid tagging result");

  const map = new Map<number, string[]>();
  for (const item of parsed.items) map.set(item.index, item.tags);
  return map;
}
...
function buildAnswer(people: PersonRecord[], tagMap: Map<number, string[]>): PersonAnswer[] {
  ...
  for (let i = 0; i < people.length; i++) {
    ...
    const p = people[i];
    result.push({ name: p.name, surname: p.surname, gender: "M", born: p.born, city: p.city, tags });
  }
  return result;
}
...
async function main(): Promise<void> {
  ...
  console.log(`Otagowywanie stanowisk (${config.llm.model})...`);
  ...
  const suspectsPath = join(process.cwd(), "hub/tasks/shared/data/suspects.json");
