...
const DOC_BASE = `${process.env.HUB_BASE_URL ?? ""}/dane/doc`;
...
async function extractRouteCode(imageBase64: string, mediaType: string): Promise<string> {
  const response = await client.messages.create({
    ...
    messages: [{
      role: "user",
      content: [
        { type: "image", source: { type: "base64", media_type: mediaType as "image/png" | "image/jpeg" | "image/gif" | "image/webp", data: imageBase64 } },
        { type: "text", text: "This is a list of blocked railway routes in the SPK system. Find the route code for the route connecting Gdańsk and Żarnowiec. Return ONLY the route code (e.g. X-01), nothing else." },
      ],
    }],
  });
...
function fillDeclaration(routeCode: string): string {
  const wdp = Math.ceil((2800 - 1000) / 500);
